...
# opcjonalny komponent do obsługi kliknięć na wykresie
try:
...
def categorize_request_type(label):
    return "API Request" if isinstance(label, str) and label.startswith('/') else "Process"
...
def filter_requests_by_type(df, include_api=True, include_processes=True, label_column='label'):
    ...
    if include_api and include_processes:
        ...
        return df_filtered[df_filtered[label_column].astype(str).str.startswith('/')]
...
def get_request_type_counts(df_list, label_column='label'):
    ...
    api_count = sum(1 for label in unique_labels if isinstance(label, str) and label.startswith('/'))
    process_count = sum(1 for label in unique_labels if not (isinstance(label, str) and label.startswith('/')))
...
def plot_general_data(df_list, selected_labels=None, chart_type="Response Times", skip_empty_dates=False,
    ...
    if chart_type == "Response Times":
        if skip_empty_dates:
            ...
            fig = px.line(response_df, x='minute', y='elapsed', color='label',
                          title='Czasy odpowiedzi w czasie (średnia co minutę)',
                          labels={'elapsed': 'Czas odpowiedzi (ms)', 'minute': 'Czas'}, markers=True)
        else:
            combined_df_sorted = combined_df.sort_values(['label', 'timeStamp'])
            fig = px.line(combined_df_sorted, x='timeStamp', y='elapsed', color='label',
                          title='Czasy odpowiedzi w czasie',
                          labels={'elapsed': 'Czas odpowiedzi (ms)', 'timeStamp': 'Czas'}, markers=True)
        ...
        fig = px.line(throughput_df, x='minute', y='count', color='label',
                      title='Throughput (żądania/minutę)',
                      labels={'count': 'Liczba żądań', 'minute': 'Czas'}, markers=True)
    elif chart_type == "Success Rate":
        combined_df['minute'] = combined_df['timeStamp'].dt.floor('1min')
        success_df = combined_df.groupby(['minute', 'label']).agg({'success': ['count', 'sum']}).reset_index()
        ...
        fig = px.line(success_df, x='minute', y='success_rate', color='label',
                      title='Wskaźnik sukcesu (%)',
                      labels={'success_rate': 'Wskaźnik sukcesu (%)', 'minute': 'Czas'}, markers=True)
...
def plot_aggregate_data(df_list, selected_labels=None, metric="Average", chart_style="Po requestach",
    ...
    if chart_style == "Po requestach":
        fig = px.line(combined_df, x='test_date_str', y=metric, color='Label',
                      title=f'{metric} w czasie dla różnych requestów',
                      labels={metric: f'{metric} (ms)', 'test_date_str': 'Data testu'}, markers=True)
    else:
        grouped_df = combined_df.groupby(['test_date_str', 'file_name'])[metric].mean().reset_index()
        fig = px.line(grouped_df, x='test_date_str', y=metric, color='file_name',
                      title=f'Średnia {metric} w czasie (zagregowane po plikach)',
                      labels={metric: f'Średnia {metric} (ms)', 'test_date_str': 'Data testu'}, markers=True)
...
def create_summary_stats(df_list, file_type, include_api=True, include_processes=True):
    ...
    if file_type == 'general':
        ...
        stats.columns = ['Średnia (ms)', 'Mediana (ms)', 'Min (ms)', 'Max (ms)', 'Odch. std', 'Success Rate', 'Liczba próbek']
    else:
        combined_df = filter_requests_by_type(combined_df, include_api, include_processes, 'Label')
        if combined_df.empty:
            return None
        stats = combined_df[['Label', 'request_type', 'Average', 'Median', '90% Line',
                             '95% Line', '99% Line', 'Error %', 'Throughput',
